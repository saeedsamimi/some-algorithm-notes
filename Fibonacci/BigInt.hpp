...
get_larger_and_smaller(const std::string &num1, const std::string &num2) {
...
BigInt BigInt::operator*(const BigInt &num) const {
  ...
  else if (is_power_of_10(
               this->value)) { // if LHS is a power of 10 do optimised operation
    product.value = num.value;
    product.value.append(this->value.begin() + 1, this->value.end());
  } else if (is_power_of_10(
                 num.value)) { // if RHS is a power of 10 do optimised operation
...
BigInt BigInt::operator%(const BigInt &num) const {
  ...
  else if (is_power_of_10(num.value)) { // if num is a power of 10 use optimised
                                        // calculation
