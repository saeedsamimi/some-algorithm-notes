...
int main() {
  ...
  for (int i = 1; i <= 10; i++)
    cout << i << ": \t" << f.evaluate(i) << endl;
  
  PolynomialFunction g({new TermFunction{1, 1},
                        new TermFunction{-0.1, 2},
                        new TermFunction{4, 3}});
  
  cout << "x - 0.1x^2 + 4x^3 for [1,10]: " << endl;
  
  for (int i = 1; i <= 10; i++)
    cout << i << ": \t" << g.evaluate(i) << endl;
  
  SumFunction h(new TermFunction{.5, 2},
                new ProductFunction{new ConstantFunction{-10}, new SinFunction{new TermFunction{1, 1}}});
  
  cout << "x^2/2 - 10sin(x): " << endl;
  for (int i = 1; i <= 10; i++)
    cout << i << ": \t" << h.evaluate(i) << endl;
}
