...
class Matrix
{
    ...
    static_assert(std::is_arithmetic_v<T>, "Matrix elements must be an integral type");
...
public:
    ...
    constexpr Matrix<Rows, OtherCols, T> operator*(const Matrix<Cols, OtherCols, T> &other) const
    {
    ...
    constexpr T determinant() const {
        ...
        if constexpr (Rows == 1) {
            ...
            for (std::size_t j = 0; j < Cols; ++j) {
                det += (this->operator()(0, j) * minor(0, j).determinant() * (j % 2 == 0 ? 1 : -1));
            }
            return det;
        }
    }

    constexpr Matrix<Rows - 1, Cols - 1, T> minor(std::size_t row, std::size_t col) const
    {
        static_assert(Rows > 1 && Cols > 1, "Matrix must be larger than 1x1 to compute minor.");
    ...
    constexpr MatrixPair lu() const
    {
        ...
        for (std::size_t i = 0; i < Rows; ++i)
        {
            ...
            for (std::size_t j = i + 1; j < Rows; ++j)
            {
                if (U(i, i) == 0)
                {
                    throw std::runtime_error("LU decomposition requires a non-singular matrix");
                }
    ...
    constexpr MatrixPair ru() const
    {
        ...
        for (std::size_t i = 0; i < Rows; ++i)
        {
            for (std::size_t j = i + 1; j < Rows; ++j)
            {
                if (R(i, i) == 0)
                {
                    throw std::runtime_error("RU decomposition requires a non-singular matrix");
                }
