...
graph2D::graph2D(int width, int height) : width(width), height(height) {
  ...
  for (int i = 0; i < height; i++) surrounded[i] = new bool[width]{false};
}

void graph2D::addBarrier(int sx, int sy, int ex, int ey) {
  for (int y = sy; y <= ey; y++)
    for (int x = sx; x <= ex; x++) surrounded[y][x] = true;
}

graph2D::~graph2D() {
  for (int i = 0; i < height; i++) delete[] surrounded[i];
  delete[] surrounded;
}
...
int graph2D::bfs(point s, point e) const {
  ...
  while (!q.empty()) {
    ...
    if (s == e) break;
    q.pop();
    if (s.x > 0) visit({s.x - 1, s.y});
    if (s.x + 1 < width) visit({s.x + 1, s.y});
    if (s.y > 0) visit({s.x, s.y - 1});
    if (s.y + 1 < height) visit({s.x, s.y + 1});
  }
  int temp = 0;
  if (visited[e.y][e.x]) {
    for (point v = parent[e.y][e.x]; v.x != -1; v = parent[v.y][v.x]) temp++;
  }
  for (int i = 0; i < height; i++) {
    delete[] visited[i];
    delete[] parent[i];
...
int graph2D::getPath(point s, point e) const { return bfs(s, e); }

bool point::operator==(point &p) const { return p.x == x && p.y == y; }
